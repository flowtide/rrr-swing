...
기동 뒤 세션이 **구독을 선언**해야 이벤트가 온다. 선언이 비면 아무것도 배달되지 않는다.

```bash
python3 bin/subscribe.py set --symbols 005930,000660 \
  --events support_enter,support_return,support_break,resistance_enter,resistance_return,resistance_break,heartbeat,macro \
  --sessions PRE_NXT,PRE_TO_REG_BREAK,REG_KRX_NXT,REG_TO_POST_BREAK,POST_NXT \
  --expires 2026-09-15T20:00:00 --append-ledger
```
...
**인바운드 어댑터**(`bin/inbound_rrr.py` — pid `local/inbound.pid`, 로그 `local/inbound.log`)는 배달만 한다. 판단·주문·`/context` 호출은 세션(LLM)의 일이며 어댑터 코드에 없다. 서버 필터(쿼리)는 편의이고 로컬 구독 재검사가 권위다.
