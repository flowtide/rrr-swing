...
| 명령 | 용도 |
|---|---|
| `bin/start.sh [--role lead|exec] [--runtime claude|agy] [--adapter-only]` | config 검증(비밀 미출력, `gw.base_url`·`gw.api_key` 존재만 확인) → selftest → `/api/health` 1회 → 내 uid 의 이전 인바운드 어댑터 전부 정리 → 인바운드 어댑터 백그라운드(`bin/detach.py` 로 자기 세션 분리 — claude 의 프로세스 그룹에 묶이지 않는다. `local/inbound.pid`, lead 만) → 런타임 기동. `operator_channel=telegram` 이면 텔레그램 플러그인 채널 주입 |
