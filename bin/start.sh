...
#
# 이 스크립트가 지키는 것:
# - 런타임은 claude 고정. 역할은 herdr 에이전트 이름으로 나눈다(D15).
...
ROLE="lead"; ADAPTER_ONLY=0; DELIVER="herdr"; SOURCE=""; SINCE=""; CHECK_ONLY=0; DRY_LAUNCH=0
while [ $# -gt 0 ]; do
  case "$1" in
    --role) ROLE="$2"; shift 2 ;;
...
if [ "$CHECK_ONLY" = "1" ]; then
  echo "check-only ok mode=$MODE role=$ROLE runtime=claude source=$SOURCE operator_channel=$OPCH account=$(cfg account_id)"
  exit 0
fi
...
if [ "$ROLE" = "lead" ]; then
  ...
  BOOT="세션 기동(role=lead, runtime=claude). AGENTS.md 계약을 적용하라. $MODE_NOTE.
...
이후 입력: 매 턴 앞에 '[턴 컨텍스트]' 한 묶음(mode·운영자 지시·태도·열린 협의 — 상태 파일에서 그때그때 읽는다), '[digest …]' 다이제스트(소스=$SOURCE), '[market-check ts=HH:MM]' 30분 시장 체크(시각뿐 — 시장은 직접 본다), 운영자 지시(채널=$OPCH; console 이면 이 pane 직접 입력, telegram 이면 텔레그램 채널), 스케줄, 회계 flag."
  ...
  BOOT="세션 기동(role=exec, runtime=claude). $MODE_NOTE.
...
export RS_ROLE="$ROLE" RS_RUNTIME="claude"
...
# MCP 는 운영자의 user scope 등록(`claude mcp add`)을 그대로 쓴다. 이 저장소는 MCP 설정을
# 만들지도 좁히지도 않는다.
# macOS 의 /bin/bash 는 3.2 다 — set -u 에서 빈 배열 전개가 unbound 로 죽는다(4.4+ 에서 고쳐진 동작).
# ${A[@]+"${A[@]}"} 는 3.2 에서도 안전하다. `--` 가 옵션 파싱을 끝내 프롬프트를 위치 인자로 고정한다.
exec claude ${EXTRA_ARGS[@]+"${EXTRA_ARGS[@]}"} -- "$BOOT"
