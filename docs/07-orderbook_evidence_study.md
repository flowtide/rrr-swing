...
### 3.1 체결 우위(매수/매도 우위) 및 호가 잔량 결합 주문가(Maker vs Taker) 결정 규칙

체결 분석에서 도출되는 **체결 우위(예: 매수 체결 비중 53%+)**와 호가창 잔량/흐름(`bid_flow`, `ask_flow`, `l1_imbalance`)을 결합하면, 슬리피지를 줄이고 1호가라도 유리하게 체결시키는 정밀 지정가(`px`)를 결정할 수 있다.

| 구분 | 체결 우위 및 호가 상태 | 권장 주문 호가 (`px`) | 집행 성격 및 판단 근거 |
|---|---|---|---|
| **매수 (1)** | **매수 우위 지속** + 매도벽 소진<br>(`ask_withdrawal`, 매도잔량 감소) | **최우선 매도호가 (Ask 1)** | **[적극적 체결 / Taker]**<br>체결 모멘텀이 강하므로 아래 호가에 걸면 체결되지 않고 상방 이탈하는 **기회비용(Execution Failure)** 방지. |
| **매수 (2)** | **매수 우위이나 가격 정체** + 매수벽 흡수<br>(`bid_replenishment`, 매수잔량 적층) | **최우선 매수호가 (Bid 1)** 또는 **1호가 아래 (Bid 2)** | **[수동적 대기 / Maker]**<br>1~2틱 싸게 매수(스프레드 이익 취득). 단, 매도벽이 계속 보충되는 `sell_absorption` 국면이면 매수 보류(§4.3 함정). |
| **매수 (3)** | **매도 우위 반전** 또는 매수벽 철수<br>(`bid_withdrawal`) | **주문 보류 (`decision(hold)`)** 또는 **Bid 2~3 아래 깊은 대기** | **[역선택(Adverse Selection) 방지]**<br>순간적인 매도 투매 물량을 받아 사자마자 물리는 위험 차단. |
| **매도 (1)** | **매수 우위 지속** + 체결강도 상승<br>(매수세가 가격을 밀어올림) | **최우선 매도호가 (Ask 1)** 또는 **1~2호가 위 (Ask 2~3)** | **[유리한 지정가 / Maker]**<br>밀고 들어오는 매수 호가에 1~2호가 더 비싸게 체결시켜 익절 수익 극대화. |
| **매도 (2)** | **매도 우위 반전** + 매도 체결 급증<br>(손절선 접근 또는 목표 달성 직후) | **최우선 매수호가 (Bid 1)** | **[신속 청산 / Taker]**<br>1호가 아끼려다 매도 잔량이 쌓이며 호가가 주저앉아 더 큰 슬리피지가 나는 참사 방지. |

#### 미시구조(Microstructure) 집행 시 필수 점검 사항
1. **흡수(Absorption) 함정 점검**: 체결 비율이 53%~90% 매수우위라도, 가격이 오르지 못하고 매도 잔량이 계속 늘어나면(`ask_flow > 0`, `sell_absorption`) 상투 매수 금지 (§4.3 참조).
2. **역선택(Adverse Selection) 주의**: 호가 아래(Bid 1~2)에 걸어둔 주문이 체결되었다는 것은, 순간적으로 대량 매도세가 쏟아져 내 매수벽을 뚫었다는 뜻일 수 있으므로 체결 직후 확정봉의 지지 여부를 재검증한다.
3. **스프레드(`avg_spread`) 고려**: 호가 갭이 넓은 얇은 종목은 대기 지정가(Maker)가 유리하나 미체결 시 취소/정정 규율이 필요하며, 호가가 촘촘한 대형주는 Taker 슬리피지 비용이 낮다.
